...
var context = {
	...
	fail: function(err) { if(err){ console.log(err); console.log(process.argv);} process.exit(); },
...
// node _container.js '{"foo": "bar"}'  node, _container.js,W29iamVjdCBKU09OXQ==
// ^0   ^1            ^2
if(process.argv[2] !== undefined) {
	console.log("process.argv[2]",process.argv[0]);
	...
	if(base64regex.test(process.argv[2])) {
		...
		// Not base64 encoded.
		try{
			console.log("event before parse",process.argv[2])
			event = JSON.parse(process.argv[2]) || {};
			require("./index.js").handler(event, context);
		} catch(e) {
			console.log("Error parsing event message."+process.argv[2], e);
		}
	}
} else {
